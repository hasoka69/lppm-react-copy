                ...
                <div className="max-w-7xl mx-auto px-6">
                    <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-1">
                        {[
                            ...
                            { label: 'Pengkinian Capaian Luaran', active: true, route: isAdminView ? route('lppm.penelitian.pengkinian-luaran') : route('dosen.penelitian.pengkinian-luaran.index') }
                        ].map((tab, idx) => (
                            <button
                                key={idx}
                ...
                >
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div className="space-y-1">
                            <Link
                                href={isAdminView ? route('lppm.penelitian.pengkinian-luaran') : route('dosen.penelitian.pengkinian-luaran.index')}
                    ...
                    <section className="space-y-6">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-xl font-bold text-gray-900 tracking-tight">Luaran <span className="text-blue-600">Wajib</span></h3>
                        </div>
                                            ...
                                            <tr>
                                                <td colSpan={4} className="px-8 py-16 text-center">
                                                    <div className="flex flex-col items-center justify-center space-y-3 opacity-40 grayscale">
                                                        ...
                                                        <p className="text-sm font-bold text-gray-500 italic">Tidak ada luaran tambahan yang tercantum</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        ) : (
        ...
        >
            ...
            <td className="px-8 py-8">
                <div className="space-y-4">
                    <div className={`mt-2 inline-flex items-center px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest ${output.status === 'Published' || output.status === 'Accepted' || output.status === 'Granted' ? 'bg-emerald-500 text-white shadow-sm' : 'bg-blue-600 text-white shadow-sm'}`}>
                        {output.status}
                    </div>
...
function EditOutputModal({ isOpen, onClose, output, type }: { isOpen: boolean, onClose: () => void, output: any, type: string }) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // Fixed route name: added .luaran before .update
        post(route(`dosen.${type}.pengkinian-luaran.luaran.update`, output.id), {
            onSuccess: () => {
                onClose();
                toast.success('Pengkinian luaran berhasil diperbarui.');
            },
            forceFormData: true,
        });
    };
                        ...
                        <div className="px-10 py-6 bg-gradient-to-r from-gray-900 to-blue-900 flex items-center justify-between">
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-blue-100/60 font-bold uppercase tracking-widest mt-0.5">Detail & Dokumentasi Bukti</p>
                                </div>
                            </div>
