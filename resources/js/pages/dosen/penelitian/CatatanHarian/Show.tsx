                ...
                <main className="flex-1 p-6 space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-xl border border-blue-100 shadow-sm space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                            <span className="text-[10px] bg-blue-600 text-white px-2 py-0.5 rounded font-bold uppercase">{usulan.kelompok_skema}</span>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex gap-3">
                            <button
                                onClick={() => router.visit(route('dosen.penelitian.catatan-harian.index'))}
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {filteredLogs.length > 0 ? (
                                        filteredLogs.map((log, index) => (
                                            <tr key={log.id} className="hover:bg-blue-50/30 transition group align-top">
                                                <td className="px-6 py-5 text-xs text-gray-500 font-medium text-center">{index + 1}</td>
                                                <td className="px-6 py-5 text-xs text-blue-700 font-bold">
                                                        ...
                                                        <div className="mt-3 flex flex-wrap gap-2">
                                                            {log.files.map((f: any) => (
                                                                <a
                                                                    ...
                                                                    target="_blank"
                                                                    className="inline-flex items-center gap-1.5 px-2 py-1 bg-gray-50 border border-gray-100 rounded text-[9px] font-bold text-gray-600 hover:bg-blue-50 hover:text-blue-600 transition truncate max-w-[150px]"
                                                                    title={f.file_name}
                                                                >
                                                        ...
                                                        <button
                                                            onClick={() => {
                                                                if (confirm('Hapus catatan ini?')) {
                                                                    router.delete(route('dosen.penelitian.catatan-harian.destroy', log.id));
                                                                }
                                                            }}
...
function CatatanModal({ usulanId, log, onClose }: { usulanId: number, log?: any, onClose: () => void }) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (log) {
            post(route('dosen.penelitian.catatan-harian.update', log.id), {
                onSuccess: () => {
                    onClose();
                    reset();
                },
            ...
            post(route('dosen.penelitian.catatan-harian.store'), {
                onSuccess: () => {
                    onClose();
                    reset();
                },
    ...
    const deleteFile = (fileId: number) => {
        if (confirm('Hapus file ini?')) {
            router.delete(route('dosen.penelitian.catatan-harian.file.destroy', fileId), {
                preserveScroll: true
            });
        }
    };
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col scale-in">
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="relative">
                                <input
                                    type="number"
                                    min="0" max="100"
                                    className="w-full text-sm border-gray-200 rounded-lg focus:ring-blue-500 focus:border-blue-500 shadow-sm pr-10 text-right font-black"
                    ...
                    <div className="space-y-1.5">
                        ...
                        <textarea
                            className="w-full text-sm border-gray-200 rounded-lg focus:ring-blue-500 focus:border-blue-500 shadow-sm min-h-[120px]"
                    ...
                    <div className="space-y-3 pt-2 border-t border-gray-100">
                                ...
                                {log.files.map((f: any) => (
                                    <div key={f.id} className="flex items-center justify-between bg-gray-50 p-2 rounded-lg border border-gray-100 group">
                                        <div className="flex items-center gap-2 overflow-hidden">
                                            <svg className="w-3.5 h-3.5 text-blue-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                        ...
                        {data.files.length > 0 && (
                            <div className="space-y-2">
                                <p className="text-[10px] text-blue-600 font-bold uppercase tracking-tight">Berkas Baru:</p>
                                {data.files.map((file, i) => (
                                    <div key={i} className="flex items-center justify-between bg-blue-50/50 p-2 rounded-lg border border-blue-100">
                                        <div className="flex items-center gap-2 overflow-hidden">
                                            <svg className="w-3.5 h-3.5 text-blue-600 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path d="M5.5 13a3.5 3.5 0 01-.369-6.98 4 4 0 117.753-1.977A4.5 4.5 0 1113.5 13H11V9.413l1.293 1.293a1 1 0 001.414-1.414l-3-3a1 1 0 00-1.414 0l-3 3a1 1 0 001.414 1.414l1.293-1.293V13H5.5z" /><path d="M9 13h2v5a1 1 0 11-2 0v-5z" /></svg>
                                            <span className="text-[11px] text-blue-700 truncate font-black">{file.name}</span>
