...
export default function Detail({ usulan, report, outputs }: Props) {
    ...
    const handleSaveReportMetadata = (e: React.FormEvent) => {
        e.preventDefault();
        postReport(route('dosen.penelitian.laporan-kemajuan.update', report.id), {
            forceFormData: true,
            onSuccess: () => alert('Data laporan berhasil disimpan.'),
        });
    };
    ...
    const handleSaveSPTB = (e: React.FormEvent) => {
        ...
        postReport(route('dosen.penelitian.laporan-kemajuan.sptb.update', report.id), {
            forceFormData: true,
            onSuccess: () => {
            ...
            <main className={styles.contentArea}>
                ...
                <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-6 mb-8 relative overflow-hidden">
                    ...
                    <div className="space-y-2">
                        <p className="text-blue-700 text-sm font-medium">Penelitian | Tahun Pelaksanaan {usulan.tahun_pertama}</p>
                        ...
                        <form onSubmit={handleSaveReportMetadata} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                            <div className="p-6 space-y-6">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 mb-2 italic">Tuliskan secara ringkas latar belakang penelitian, tujuan dan tahapan metode, luaran yang ditargetkan, dan hasil yang diperoleh sesuai dengan tahun pelaksanaan</p>
                                    <textarea
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="Contoh: Pemrograman; Web; Sistem Informasi"
                                        disabled={report.status !== 'Draft'}
                                    />
                                </div>
                        ...
                        <div className="flex justify-between items-center pt-8">
                            ...
                            <div className="flex gap-2">
                                {report.status === 'Draft' ? (
                                    <button
                                        onClick={() => {
                                            ...
                                            if (confirm('Kirim laporan kemajuan sekarang? Data tidak dapat diubah setelah disubmit.')) {
                                                router.post(route('dosen.penelitian.laporan-kemajuan.submit', usulan.id));
                                            }
                                        }}
                                        disabled={reportProcessing}
                                    ...
                                    <button
                                        onClick={() => router.visit(route('dosen.penelitian.laporan-kemajuan.index'))}
