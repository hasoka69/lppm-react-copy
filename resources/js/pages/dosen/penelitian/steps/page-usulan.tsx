        ...
        <div className={styles.container}>
            <div className={styles.header}>
                <div className={styles.titleContainer}>
                    ...
                    <p className={styles.subtitle}>Kelola daftar usulan penelitian Anda di sini.</p>
                </div>
                <div className={styles.headerAction}>
                    {onTambahUsulan && !isPerbaikanView && (
                    ...
                    <table className={styles.table}>
                        ...
                        <tbody>
                            {filteredUsulanList.length === 0 ? (
                                ...
                                filteredUsulanList.map((usulan, index) => {
                                    const statusStyle = getStatusStyle(usulan.status);
                                    return (
                                        <motion.tr
                                            key={usulan.id}
                                        ...
                                        >
                                            ...
                                            <td className={styles.judulCell} style={{ padding: '1.25rem 0.75rem' }}>
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                                                    ...
                                                    <span style={{ fontWeight: 700, color: 'var(--secondary)', lineHeight: 1.4 }} title={usulan.judul}>
                                                        {usulan.judul}
                                                    </span>
                                                    <div style={{ display: 'flex', gap: '1rem', marginTop: '0.15rem' }}>
                                                        <span style={{ fontSize: '0.75rem', color: '#64748b', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                                            <Calendar size={12} /> {formatAcademicYear(usulan.tahun_pelaksanaan)}
                                                        </span>
                                                        <span style={{ fontSize: '0.75rem', color: '#64748b', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                                            <User size={12} /> {usulan.peran}
                                                        </span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="font-medium text-gray-600">{usulan.skema}</td>
                                            <td>
                                                <span className={styles.badge} style={{
                                                    backgroundColor: statusStyle.bg,
                                                    color: statusStyle.color,
                                                    border: `1px solid ${statusStyle.bg === '#ffffff' ? '#e2e8f0' : 'transparent'}`
                                                }}>
                                                    <span style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: statusStyle.dot, marginRight: '6px' }}></span>
                                                    {statusStyle.label}
                                                </span>
                                            </td>
                                            <td className="text-center px-4">
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end" className="w-[160px]">
                                                        <DropdownMenuItem onClick={() => onViewUsulan && onViewUsulan(usulan)}>
                                                        ...
                                                        {usulan.status === 'draft' && (
                                                            <DropdownMenuItem className="text-red-600 focus:text-red-600" onClick={() => {
                                                                if (confirm('Yakin ingin menghapus usulan ini?')) {
                                                                    onDeleteUsulan && onDeleteUsulan(usulan.id);
                                                                }
                                                            }}>
