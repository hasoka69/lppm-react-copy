...
export default function Detail({ usulan, report, outputs }: Props) {
    ...
    const handleSaveReportMetadata = (e: React.FormEvent) => {
        e.preventDefault();
        postReport(route('dosen.pengabdian.laporan-akhir.update', report.id), {
            forceFormData: true,
            onSuccess: () => alert('Data laporan akhir pengabdian berhasil disimpan.'),
        });
    };
    ...
    const handleSaveSPTB = (e: React.FormEvent) => {
        ...
        postReport(route('dosen.pengabdian.laporan-akhir.sptb.update', report.id), {
            forceFormData: true,
            onSuccess: () => {
            ...
            <main className={styles.contentArea}>
                ...
                >
                    <div>
                        ...
                        <p className="text-gray-500 mt-1">Formulir pengisian hasil akhir kegiatan pengabdian masyarakat</p>
                    </div>
                </motion.div>
                ...
                >
                    ...
                    <div className="space-y-4 max-w-4xl">
                        <div className="flex items-center gap-2">
                            ...
                            <p className="text-blue-700 text-[11px] font-bold uppercase tracking-widest">Pengabdian Masyarakat</p>
                        </div>
                        ...
                        >
                                            ...
                                            <div>
                                                ...
                                                <div className="relative">
                                                    <input
                                                        ...
                                                        placeholder="Contoh: Edukasi; Masyarakat; Inovasi"
                                                        disabled={report.status !== 'Draft'}
                                                    />
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <section className="bg-white rounded-2xl shadow-xl shadow-blue-500/5 border border-gray-100 p-8 ring-1 ring-black/5">
                                    ...
                                    <div className="space-y-6">
                                        <p className="text-sm text-gray-500 leading-relaxed italic">Wajib mengunggah poster hasil pengabdian dalam format PNG/JPG/PDF maksimal 10MB.</p>
                            ...
                            <div className="flex justify-between items-center pt-10 border-t border-gray-100">
                                <button
                                    ...
                                    onClick={() => router.visit(route('dosen.pengabdian.laporan-akhir.index'))}
                                >
...
function OutputRow({ output, index, isReadOnly }: { output: any, index: number, isReadOnly: boolean }) {
    ...
    return (
        <>
            <tr className="hover:bg-gray-50/80 transition-all align-top group">
                <td className="px-8 py-8 text-xs text-gray-400 font-black tracking-tighter w-16">{index.toString().padStart(2, '0')}</td>
                <td className="px-8 py-8">
                    <div className="space-y-3">
                        <div className="flex items-center gap-2">
                            <div className="w-2 h-2 rounded-full bg-blue-600 shadow-[0_0_8px_rgba(37,99,235,0.4)]" />
                            <p className="text-sm font-black text-gray-900 leading-none uppercase tracking-tight">{output.kategori}</p>
                        </div>
                        <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 group-hover:bg-white group-hover:border-blue-100 transition-all relative overflow-hidden">
                            <div className="absolute top-0 right-0 p-2 opacity-5 group-hover:opacity-10 transition-opacity">
                                <FileText className="w-8 h-8 text-blue-900" />
                            </div>
                            <p className="text-[11px] text-gray-500 font-medium leading-relaxed italic relative z-10">
                                {output.deskripsi}
                            </p>
                        </div>
                    </div>
                ...
                <td className="px-8 py-8">
                    <div className="space-y-4">
                        {output.judul_realisasi_akhir ? (
                            <div className="bg-emerald-50/30 rounded-2xl p-5 border border-emerald-100/50 space-y-3 relative group/card overflow-hidden">
                                ...
                                <div>
                                    <p className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.15em] mb-1">Judul Realisasi</p>
                                    <p className="text-sm text-gray-900 font-black leading-tight tracking-tight uppercase">{output.judul_realisasi_akhir}</p>
                                </div>
                                <div className="flex flex-wrap items-center gap-3">
                                    <span className="bg-blue-600 text-white text-[9px] px-2.5 py-1 rounded-lg font-black uppercase tracking-widest shadow-lg shadow-blue-500/20">
                                        {output.status_akhir}
                                    </span>
                                    {output.nama_jurnal && (
                                        <span className="flex items-center gap-1.5 text-[10px] text-gray-500 font-bold bg-white px-2 py-1 rounded-lg border border-gray-100 uppercase tracking-tight">
                                ...
                                {output.file_bukti_akhir && (
                                    <a
                                        href={`/storage/${output.file_bukti_akhir}`}
                                        target="_blank"
                                        className="inline-flex items-center gap-2 text-[10px] font-black text-blue-600 hover:text-blue-800 transition uppercase tracking-widest group/link"
                                    >
                                        Unduh Bukti Laporan
                            ...
                            <div className="flex flex-col items-center justify-center p-6 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                                ...
                                <span className="text-[10px] text-gray-400 font-black uppercase tracking-widest">Belum Ada Realisasi</span>
                            </div>
                        )}
                    </div>
                </td>
                <td className="px-8 py-8 text-center align-middle">
                    {!isReadOnly && (
                        <motion.button
                            ...
                            className="w-12 h-12 flex items-center justify-center bg-white border-2 border-gray-100 rounded-2xl text-gray-400 hover:border-blue-500 hover:text-blue-600 hover:shadow-xl hover:shadow-blue-500/10 transition-all group"
                        >
            ...
            <AnimatePresence>
                {isEditing && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-gray-900/80 backdrop-blur-md">
                        ...
                        >
                            <div className="px-10 py-8 bg-gradient-to-r from-gray-900 to-blue-900 flex items-center justify-between relative overflow-hidden">
                                <div className="absolute inset-0 opacity-10">
                                    <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500 rounded-full -translate-y-1/2 translate-x-1/2 blur-[100px]" />
                                    <div className="absolute bottom-0 left-0 w-64 h-64 bg-indigo-500 rounded-full translate-y-1/2 -translate-x-1/2 blur-[100px]" />
                                </div>
                                <div className="relative z-10">
                                    <p className="text-blue-400 text-[10px] font-black uppercase tracking-[0.3em] mb-2 leading-none">Formulir Capaian</p>
                                    <h3 className="text-xl font-black text-white uppercase tracking-tight leading-none leading-none">
                                        Realisasi: {output.kategori}
                                    </h3>
                                </div>
                                <button onClick={() => setIsEditing(false)} className="relative z-10 w-10 h-10 border border-white/10 rounded-xl flex items-center justify-center text-white/50 hover:text-white hover:bg-white/10 transition">
                            ...
                            <div className="flex-1 overflow-y-auto p-10 space-y-10">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-2">
                                        <label className="block text-[11px] font-black text-gray-500 uppercase tracking-widest ml-1">Status Capaian</label>
                                        <select
                                            className="w-full bg-gray-50 border-gray-100 rounded-2xl py-4 px-5 text-sm font-bold focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all appearance-none"
                                        ...
                                        >
                                            ...
                                            <option value="Review">Review (Penelaahan)</option>
                                            <option value="Accepted">Accepted (Diterima)</option>
                                            <option value="Published">Published (Terbit/Selesai)</option>
                                        </select>
                                    </div>
                                    <div className="space-y-2">
                                        <label className="block text-[11px] font-black text-gray-500 uppercase tracking-widest ml-1">Peran Dalam Program</label>
                                        <input
                                            type="text"
                                            className="w-full bg-gray-50 border-gray-100 rounded-2xl py-4 px-5 text-sm font-bold focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all"
                                            value={data.peran_penulis}
                                            onChange={e => setData('peran_penulis', e.target.value)}
                                            placeholder="Contoh: Ketua Tim / Narasumber"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-2">
                                        <label className="block text-[11px] font-black text-gray-500 uppercase tracking-widest ml-1">Nama Jurnal/Media/Mitra</label>
                                        <input
                                            type="text"
                                            className="w-full bg-gray-50 border-gray-100 rounded-2xl py-4 px-5 text-sm font-bold focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all"
                                            value={data.nama_jurnal}
                                            onChange={e => setData('nama_jurnal', e.target.value)}
                                            placeholder="Contoh: Jurnal Inovasi Masyarakat"
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="block text-[11px] font-black text-gray-500 uppercase tracking-widest ml-1">URL Publikasi / Bukti Digital</label>
                                        <input
                                            type="url"
                                            className="w-full bg-gray-50 border-gray-100 rounded-2xl py-4 px-5 text-sm font-bold focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all"
                                ...
                                <div className="space-y-2">
                                    <label className="block text-[11px] font-black text-gray-500 uppercase tracking-widest ml-1">Judul / Deskripsi Realisasi</label>
                                    <textarea
                                        className="w-full bg-gray-50 border-gray-100 rounded-2xl py-4 px-5 text-sm font-bold focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all min-h-[120px] resize-none"
                                        value={data.judul_realisasi_akhir}
                                        onChange={e => setData('judul_realisasi_akhir', e.target.value)}
                                        placeholder="Tuliskan judul artikel atau deskripsi mendalam mengenai realisasi luaran ini..."
                                    />
                                </div>

                                <div className="bg-blue-50/50 rounded-3xl p-8 border border-blue-100/50">
                                    <div className="flex items-center justify-between mb-6">
                                        <h4 className="text-sm font-black text-blue-900 uppercase tracking-tight">Dokumen Bukti Fisik</h4>
                                        <p className="text-[10px] text-blue-600 font-bold uppercase tracking-widest">PDF Maksimal 10MB</p>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            ...
                                            <div className="bg-white border border-blue-100 rounded-2xl p-4 flex items-center justify-between shadow-sm">
                                                <div className="flex items-center gap-3 overflow-hidden">
                                                    ...
                                                    <div className="overflow-hidden">
                                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Berkas Tersimpan</p>
                                                        <p className="text-xs font-bold text-gray-700 truncate">{output.file_bukti_akhir.split('/').pop()}</p>
                                                    </div>
                                                </div>
                                                <a href={`/storage/${output.file_bukti_akhir}`} target="_blank" className="p-2 hover:bg-blue-50 rounded-lg transition text-blue-600">
                                        ...
                                        <div>
                                            ...
                                            <button
                                                ...
                                                className={`w-full h-full flex items-center justify-center gap-3 py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition border-2 border-dashed ${data.file_bukti_akhir ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'bg-white border-blue-600 text-blue-600 hover:bg-blue-50'}`}
                                            >
                                                {data.file_bukti_akhir ? <CheckCircle2 className="w-5 h-5" /> : <Upload className="w-5 h-5" />}
                                                {data.file_bukti_akhir ? 'Berkas Dipilih' : 'Unggah Bukti Baru'}
                                            </button>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="px-10 py-8 bg-gray-50 border-t border-gray-100 flex justify-end gap-4">
                                <button
                                    ...
                                    className="px-8 py-4 bg-white border border-gray-200 text-gray-500 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-gray-100 transition"
                                >
                                    Batalkan
                                </button>
                                <button
                                    ...
                                    disabled={processing}
                                    className="px-12 py-4 bg-blue-600 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-xl shadow-blue-500/20 hover:bg-blue-700 transition disabled:opacity-50"
                                >
