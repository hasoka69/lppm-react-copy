...
export default function Detail({ usulan, laporan_kemajuan, outputs, isAdminView = false }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('dosen.pengabdian.laporan-kemajuan.update', laporan_kemajuan.id), {
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                if (confirm('Review kembali semua data. Setelah finalisasi, data tidak dapat diubah. Lanjutkan?')) {
                                                    post(route('dosen.pengabdian.laporan-kemajuan.submit', usulan.id), { forceFormData: true });
                                                }
                                            }}
...
function OutputRow({ output, index, isAdminView = false, isLaporanDraft = true }: { output: any, index: number, isAdminView?: boolean, isLaporanDraft?: boolean }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        // tahun_realisasi: output.kemajuan_data?.tahun_realisasi || output.tahun_realisasi || '',
                        ...
                        <div className="space-y-4">
                            <div className="p-4 bg-gray-50 border border-gray-100 rounded-xl group-hover:bg-white transition-colors">
                                ...
                                <div className="flex items-center gap-4 mt-3">
                                    ...
                                    {(output.kemajuan_data?.url_artikel || output.url_artikel) && (
                                        <a href={output.kemajuan_data?.url_artikel || output.url_artikel} target="_blank" className="flex items-center gap-1.5 text-indigo-600 hover:text-indigo-800 transition-colors">
