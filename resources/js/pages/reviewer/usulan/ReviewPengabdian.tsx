...
interface RabItem {
    ...
    satuan: string;
    total: number;
    tipe: string; // bahan, perjalanan, publikasi, pengumpulan_data, sewa_peralatan
}
...
interface PageProps {
    proposal: Proposal;
    dosen: Dosen;
    mitra: Mitra[]; // Array needed here
    anggotaNonDosen: AnggotaNonDosen[]; // Added this
    rabItem: RabItem[];
    luaran: Luaran[]; // [NEW] Pass explicitly
...
export default function ReviewPengabdian({ proposal, dosen, mitra = [], anggotaNonDosen = [], rabItem = [], luaran = [], totalAnggaran = 0 }: PageProps) {
    ...
    // Calculate RAB items (using props rabItem or proposal.rabItems if controller passed differently)
    ...
    // Luaran Items (Provided via props)

    console.log('ReviewPengabdian Props:', { proposal, luaran }); // DEBUGGING luaran listing

    // Mitra - Ensure it's an array for rendering (even if controller sends single obj or array)
    // Sometimes backend might send array even if property name is singular
        ...
        <div className="min-h-screen bg-gray-50 font-sans relative">
            ...
            <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <section className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8 text-sm">
                                ...
                                {/* New Fields: Bidang Fokus & Rumpun Ilmu */}
                                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-3 rounded-lg border border-gray-100">
                                    <div>
                                        <span className="block text-gray-500 text-xs uppercase tracking-wide mb-1">Bidang Fokus ({proposal.jenis_bidang_fokus})</span>
                                        <span className="font-medium text-gray-800 flex items-center">
                                            <Target className="w-3 h-3 mr-1 text-blue-500" />
                                            {proposal.bidang_fokus || '-'}
                                        </span>
                                    </div>
                                    <div>
                        ...
                        <section className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            ...
                            {proposal.file_substansi ? (
                                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 mb-6">
                                    ...
                                    <a
                                        href={`/storage/${proposal.file_substansi}`}
                                        target="_blank"
                            ...
                            {items.length > 0 ? (
                                <div className="space-y-6">
                                    {Object.entries(rabGroups).map(([group, groupItems]) => (
                                        groupItems.length > 0 && (
                                            <div key={group}>
                                                <h4 className="text-sm font-semibold text-gray-700 mb-2 uppercase tracking-wide border-l-4 border-blue-500 pl-2 bg-gray-50 py-1">
                                                    {group}
                                                </h4>
                                                <div className="overflow-x-auto">
                                                    <table className="min-w-full divide-y divide-gray-200 text-sm">
                        ...
                        <section className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            ...
                            {mitras.length > 0 ? (
                                    ...
                                    {mitras.map((m, index) => (
                                        <div key={m.id} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div>
                                                    <p className="text-xs text-gray-500 uppercase">Nama Mitra</p>
                                            ...
                                            {m.file_surat_kesediaan ? (
                                                <div className="mt-4 flex items-center justify-between p-3 bg-white border border-blue-100 rounded-lg shadow-sm">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-sm text-blue-900">Surat Kesediaan Mitra.pdf</span>
                                                    </div>
                                                    <a href={`/storage/${m.file_surat_kesediaan}`}
                                                        target="_blank"
                                                        rel="noreferrer"
                                ...
                                <div className="p-4 bg-red-50 text-red-800 rounded-lg text-sm flex items-center">
                                    <XCircle className="w-5 h-5 mr-2" />
                                    Data Mitra Sasaran belum diisi.
                                </div>
                            )}
                        </section>
                    ...
                    {/* RIGHT COLUMN: ACTION FORM (PAGE 5 / REVIEW) */}
                            ...
                            <form className="space-y-6">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <button
                                        ...
                                        disabled={processing}
                                        className="w-full flex justify-center items-center px-4 py-3 border border-transparent rounded-md shadow-sm text-sm font-bold text-white bg-orange-500 hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 disabled:opacity-50 transition-all"
                                    >
                                        <AlertCircle className="w-5 h-5 mr-2" />
                                        Minta Revisi
                                    </button>
                                    ...
                                    <button
                                        ...
                                        disabled={processing}
                                        className="w-full flex justify-center items-center px-4 py-3 border border-transparent rounded-md shadow-sm text-sm font-bold text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:opacity-50 transition-all"
                                    >
                                        <XCircle className="w-5 h-5 mr-2" />
                                        Tolak Permanen
                                    </button>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
                        <div className="bg-white rounded-lg max-w-md w-full p-6 shadow-2xl transform transition-all animate-in fade-in zoom-in-95">
                            ...
                            <p className="text-gray-600 mb-6 text-sm leading-relaxed">
                                {actionType === 'approve'
                                    ? 'Anda yakin ingin MENYETUJUI usulan ini? Usulan akan ditandai sebagai "Didanai" dan proses review selesai.'
                                    : actionType === 'revise'
