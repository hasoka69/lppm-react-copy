...
class LaporanAkhirPenelitianController extends Controller
{
    public function index()
    {
        ...
        return Inertia::render('dosen/penelitian/LaporanAkhir/Index', [
    ...
    {
        // For final report, we only show dedicated outputs
        $usulan = UsulanPenelitian::with(['luaranList'])
            ->where('user_id', '=', Auth::id(), 'and')
            ->where('status', '=', 'didanai', 'and')
            ->findOrFail($usulanId);

        $report = LaporanAkhirPenelitian::firstOrCreate(
            ['usulan_id' => $usulan->id],
    ...
    public function update(Request $request, $id)
    {
        ...
        $report = LaporanAkhirPenelitian::where('user_id', '=', Auth::id(), 'and')->findOrFail($id);
        ...
        if ($request->hasFile('file_laporan')) {
            ...
            $path = $request->file('file_laporan')->store('laporan_akhir_penelitian', 'public');
        ...
        if ($request->hasFile('file_poster')) {
            ...
            $path = $request->file('file_poster')->store('laporan_akhir_penelitian/poster', 'public');
    ...
    public function updateLuaran(Request $request, $luaranId)
    {
        ...
        $report = LaporanAkhirPenelitian::where('usulan_id', '=', $luaran->usulan_id, 'and')->first();
    ...
    public function updateSPTB(Request $request, $id)
    {
        ...
        $report = LaporanAkhirPenelitian::findOrFail($id);
        ...
        if ($request->hasFile('file_sptb')) {
            ...
            $path = $request->file('file_sptb')->store('laporan_akhir_penelitian/sptb', 'public');
    ...
    {
        $report = LaporanAkhirPenelitian::where('usulan_id', '=', $usulanId, 'and')
            ->where('user_id', '=', Auth::id(), 'and')
            ->firstOrFail();
        ...
        return redirect()->route('dosen.penelitian.laporan-akhir.index')
